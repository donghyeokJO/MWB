    ...
    class Iamport
    {
        ...
        public function findByMerchantUID($merchantUid, $statusFilter = null)
        {
            try {
                ...
                if (in_array($statusFilter, ['ready', 'paid', 'cancelled', 'failed'])) {
        ...
        public function sbcr_onetime($data)
        {
            try {
                $access_token = $this->getAccessCode();
                $keys = array_flip(['token', 'merchant_uid', 'amount', 'vat', 'card_number', 'expiry', 'birth', 'pwd_2digit', 'customer_uid', 'name', 'buyer_name', 'buyer_email', 'buyer_tel', 'buyer_addr', 'buyer_postcode', 'card_quota', 'custom_data', 'notice_url']);
        ...
        protected function postResponse($request_url, $post_data = [], $headers = [])
        {
        ...
        protected function deleteResponse($request_url, $headers = [])
        {
